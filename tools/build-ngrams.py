...
Output (v2, the scored-prediction format): per head one compact
string, "T succ|c succ|c ...", where T and c are log-quantized
counts (code = round(ln(count) * QUANT_K); decode exp(code/QUANT_K)).
T is the head's total adjacency count BEFORE the cap and floor, so
c/T approximates the true conditional probability. The five-chip
strip needs no more precision than the ~13% quantization step.
Heads stay in vocabulary frequency order (the predictor's
first-wins key-collision rule depends on it).
...
def main():
    ...
    for head in words:  # vocabulary frequency order
        ...
        if ranked:
            succ = ' '.join(f'{w}|{quant(-nc)}'
                            for nc, w in ranked[:TOP_SUCCESSORS])
            successors[head] = f'{quant(totals[head])} {succ}'
    ...
    header = (
        ...
        f"// v2 format, per {lang} head: \"T succ|c ...\", T = total\n"
        f"// adjacency count, c = pair count, both log-quantized\n"
        f"// (decode exp(code/{QUANT_K})). Top {TOP_SUCCESSORS} successors,\n"
    ...
    kept = sum(v.count('|') for v in successors.values())
