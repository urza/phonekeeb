...
Output format matches bigrams v2, keyed by "w1 w2":
  {"you are": "T succ|c succ|c ...", ...}
with T and c log-quantized (decode exp(code/QUANT_K), same QUANT_K as
build-ngrams.py). Adjacency, holdout, and tokenization rules are
identical to build-ngrams.py (tokenization imported, never copied).
"""
import gzip
...
def main():
    ...
    for (a, b), ci in kept.items():  # insertion order = corpus order
        ...
        if not ranked:
            continue
        succ = ' '.join(f'{w}|{quant(-nc)}' for nc, w in ranked[:TOP_SUCCESSORS])
        out[f'{a} {b}'] = f'{quant(contexts[(a, b)])} {succ}'
    ...
    header = (
        ...
        f"// {lang} trigram layer, per context \"w1 w2\": \"T succ|c ...\",\n"
        f"// T = context count, c = triple count, log-quantized (decode\n"
    ...
    n_succ = sum(v.count('|') for v in out.values())
