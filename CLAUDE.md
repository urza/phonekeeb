...
- Five pages, each with its own entry script: `index.html`/`main.js`
  ...
  keyboard learned), and `bench.html`/`bench.js` (strip latency on the
  phone itself, the twin of `tools/bench-predict.mjs`; vocabulary size
  is a latency question first, so both harnesses split the numbers by
  prefix length). The dictionary page keeps every mutation in
...
Word lists: `words-en.js` / `words-cs.js` (core, 3000, eager) and
`words-ext-en.js` / `words-ext-cs.js` (the tail to 50000 English and
150000 Czech, unigram-only, lazy-loaded; the two languages run out of
...
counts (which keeps the spoken words: `uh`, `bye`, `pojď`, `počkej`).
HOW they rank is the OpenSubtitles probability alone, add-one smoothed,
because the strip must rank in the register it is typed in. Ranking by
the blend was tried and reverted the same day: it put `the` in front of
`you` and dropped `hello` below `hell`. See
`word-prediction-research.md`, "The corpus was the bug". The tail is aspell-filtered, with three
documented exceptions in `in_dict`/`wordfreq_lists`. Attribution:
wordfreq data is CC-BY-SA 4.0 and credits SUBTLEX; OpenSubtitles is
https://www.opensubtitles.org. The dump modes of the same script still
build `bigrams-*.js` and `trigrams-*.js`, which must be regenerated
whenever the core list changes: `prediction.js` assumes the n-gram
vocabulary is exactly the core tier. One-letter words and in-word
apostrophes are deliberate; see the "Vocabulary bug" section of
`word-prediction-research.md` before any filter change.
