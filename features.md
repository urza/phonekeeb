...
- Bigram tables v2: `bigrams-en.js` / `bigrams-cs.js`, generated by
  ...
  count plus the top 24 successors with pair counts, log-quantized
  ...
  tier was top 12 / floor 4 on an 80 MiB prefix, and the sweep showed
  the corpus size alone changes nothing when the floor scales with
  it: the gain is pure successor depth, +3.7pp EN / +2.6pp CS prefix
  hit@1 for 403 KB gzipped against 229). Clause punctuation (.!?…)
...
- Measured on held-out subtitles (tools/eval-prediction.mjs, hit@3,
  mixed model, trigram tables on, 2026-08-27 sweep): next-word 36% EN
  / 33% CS, two-letter prefix 75% EN / 71% CS, corrupted prefix 53%
  EN / 48% CS. The single-language ceilings sit within a point of the
...
| Bigram successors per head | top 24, pair count >= 20 (400 MiB corpus), core vocabulary only |
...
| Context-miss discount | 0.15 x next level when any language knows the context (cross-language) |
